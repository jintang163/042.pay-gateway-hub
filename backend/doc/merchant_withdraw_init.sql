...
CREATE TABLE IF NOT EXISTS fee_promotion (
    ...
    target_value VARCHAR(512) COMMENT '目标值(行业编码/商户号,逗号分隔)',
    fee_type TINYINT NOT NULL DEFAULT 1 COMMENT '优惠类型 1-费率折扣 2-固定手续费 3-0手续费',
    discount_fee_rate DECIMAL(10,4) COMMENT '折扣费率(fee_type=1时有效,0~100表示百分比)',
    fixed_fee_amount DECIMAL(18,2) COMMENT '固定手续费(fee_type=2时有效,单位元)',
    max_discount_amount DECIMAL(18,2) COMMENT '单笔最高优惠金额(元)',
    start_time DATETIME COMMENT '活动开始时间',
    end_time DATETIME COMMENT '活动结束时间',
    duration_days INT COMMENT '活动持续天数(新商户有效天数)',
    total_quota INT DEFAULT 0 COMMENT '总使用次数配额,0不限制',
    used_quota INT DEFAULT 0 COMMENT '已使用次数',
    per_merchant_quota INT DEFAULT 0 COMMENT '单商户使用次数配额,0不限制',
...
-- 活动商户关联表（字段与FeePromotionMerchant实体对齐）
CREATE TABLE IF NOT EXISTS fee_promotion_merchant (
    ...
    total_quota INT DEFAULT 0 COMMENT '商户使用次数配额',
    used_quota INT DEFAULT 0 COMMENT '已使用次数',
    total_discount_amount DECIMAL(18,2) DEFAULT 0 COMMENT '累计优惠金额',
    status TINYINT DEFAULT 1 COMMENT '状态 1-有效 0-无效',
    bind_source VARCHAR(32) COMMENT '绑定来源: MANUAL/AUTO/NEW_MERCHANT',
    remark VARCHAR(256) COMMENT '备注',
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    deleted INT DEFAULT 0,
...
INSERT INTO fee_promotion (
    ...
    discount_fee_rate, fixed_fee_amount, max_discount_amount,
    start_time, end_time, duration_days,
    total_quota, per_merchant_quota,
    status, operator_id, operator_name
) VALUES (
    'FP2025001', '新商户首月0费率', '新注册商户首月享受0手续费优惠',
    1, 4, 3,
    NULL, NULL, NULL,
    NOW(), DATE_ADD(NOW(), INTERVAL 90 DAY), 30,
    0, 0,
    2, 'SYSTEM', '系统'
);
...
INSERT INTO fee_promotion_merchant (
    promotion_no, merchant_no, merchant_name,
    start_time, end_time, total_quota, used_quota, total_discount_amount,
    status, bind_source, remark
) VALUES
('FP2025001', 'M000001', '沙箱测试商户1', NOW(), DATE_ADD(NOW(), INTERVAL 30 DAY), 0, 0, 0.00, 1, 'NEW_MERCHANT', '新商户注册自动绑定'),
('FP2025001', 'M000002', '沙箱测试商户2', NOW(), DATE_ADD(NOW(), INTERVAL 30 DAY), 0, 0, 0.00, 1, 'NEW_MERCHANT', '新商户注册自动绑定');
